...
**✅ Última atualização:** 15/01/2026 - Adicionados endpoints de **upload de boleto** e **histórico de pagamentos**
...
5. [Endpoints de Banco](#-endpoints-de-banco-novo---07012026-atualizado-15012026) ⭐ **NOVO**
6. [Endpoints de Pagamentos](#-endpoints-de-pagamentos-novo---13012026) ⭐ **NOVO**
7. [Endpoints de Download](#-endpoints-de-download)
8. [Endpoints Internos](#endpoints-internos)
9. [APIs Externas Utilizadas](#apis-externas-utilizadas)
10. [Códigos de Erro](#códigos-de-erro)
11. [Arquitetura e Serviços](#arquitetura-e-serviços)
12. [Sistema de Aprendizado e Contexto Persistente](#-sistema-de-aprendizado-e-contexto-persistente)
...
## 🏦 Endpoints de Banco (NOVO - 07/01/2026, ATUALIZADO 15/01/2026)

### `POST /api/banco/upload-boleto` ⭐ **NOVO (13/01/2026)**
**Descrição:** Upload de boleto em PDF para extração dos dados e **início do pagamento** (quando possível) via Santander.

**Headers:** `multipart/form-data`

**Body (form-data):**
- `file` (**obrigatório**): arquivo PDF do boleto
- `session_id` (opcional, default: `"default"`): usado para salvar contexto do pagamento na sessão
...
```json
{
  ...
  "dados": {
    ...
    "saldo_apos_pagamento": 9100.0,
    "payment_id": "string",
    "status": "PENDING_VALIDATION",
    "pagamento_iniciado": true
  },
  "resposta": "string (preview/explicação para o usuário)",
  "payment_id": "string",
  "pagamento_iniciado": true,
...
- O endpoint **não efetiva** o pagamento sozinho: ele prepara/inicia e orienta o usuário a **confirmar** pelo chat.
- O `payment_id` retornado é a referência para efetivação/continuação do fluxo.
...
## 💳 Endpoints de Pagamentos (NOVO - 13/01/2026)

### `GET /api/pagamentos/historico`
**Descrição:** Lista histórico de pagamentos registrados no SQLite (ex: boletos/PIX/TED), com filtros opcionais.

**Query Parameters (opcionais):**
- `banco`: `SANTANDER` | `BANCO_DO_BRASIL`
- `ambiente`: `SANDBOX` | `PRODUCAO`
- `tipo`: `BOLETO` | `PIX` | `TED` | `BARCODE`
- `status`: `READY_TO_PAY` | `PENDING_VALIDATION` | `PAYED` | `CANCELLED` | `FAILED`
- `data_inicio`: `YYYY-MM-DD`
- `data_fim`: `YYYY-MM-DD`
- `limite`: número (default: `100`)
...
### ✅ DI — Ações / Itens (Ações futuras: produtos)

| Operação | API | Endpoint |
|---|---|---|
| Consultar adições da DI (paginado via header `links`) | Integra Comex (DI) | `GET /declaracao-importacao/{numeroDI}/adicoes/` |

**Paginação (header `links`):**
- `rel="self"`: requisição atual
- `rel="next"`: próxima página (ex.: `/declaracao-importacao/2000000179/adicoes/011/continuar`)

**Implementação no mAIke:**
- Service: `services/di_adicoes_service.py`
- Tool: `consultar_adicoes_di`
...
```
1. APIs oficiais (fonte da verdade)
   - Serpro / Integra Comex (CE/DI/CCT)
   - Portal Único (DUIMP)
   - ShipsGo (Tracking/ETA/POD)
   ↓
2. Kanban (derivado)
   - Alimentado pelas 3 APIs acima + inserções manuais
   ↓
3. ProcessoKanbanDTO (DTO - Data Transfer Object)
   ↓
4. SQLite (dados_completos_json + campos normalizados)
```

**Detalhamento do Fluxo:**

1. **Origem dos Dados (JSON):**
   - Kanban retorna processos em formato JSON (camada derivada)
...
- ✅ **Manutenibilidade (DTO-first):** DTO padroniza estrutura de dados e funciona como “fronteira de acoplamento”
  - Se uma API mudar, o ajuste esperado é re-acoplar **no DTO/adapters**, não espalhar mudanças no restante do sistema
