...
4. [Queries de Referência](#queries-de-referência)
5. [Problemas Críticos Resolvidos](#problemas-críticos-resolvidos)
6. [Mapeamento de Campos Críticos](#mapeamento-de-campos-críticos)
...
## 🧾 Vendas (Make/Spalla) → Persistência no `mAIke_assistente` (PLANEJADO)

**Objetivo (resumo):** tratar a consulta de “vendas por NF” do legado (Make/Spalla) como uma “API” e **persistir um snapshot normalizado** no `mAIke_assistente` para permitir:
- filtros e refinamentos iterativos (cliente/data/devolução) sem reconsultar o legado
- Curva ABC (cliente/centro/empresa/operação)
- auditoria/rasteabilidade (chave do documento no legado + hash + JSON bruto opcional)

**Tabela planejada (MVP):** `mAIke_assistente.dbo.VENDAS_DOCUMENTO`  
**Detalhes completos:** ver `docs/MAPEAMENTO_SQL_SERVER.md` → seção **“VENDAS_DOCUMENTO (PLANEJADO)”**.
