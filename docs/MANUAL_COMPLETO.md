...
12. [Sincronização e Conciliação Bancária](#-sincronização-e-conciliação-bancária-novo---07012026) ⭐ **NOVO**
13. [Normalização de Termos Cliente → Categoria](#-normalização-de-termos-cliente--categoria-novo---08012026) ⭐ **NOVO**
14. [Streaming de Respostas](#-streaming-de-respostas-novo---05012026) ⭐ **NOVO**
15. [Assistants API para Legislação](#-assistants-api-para-legislação-novo---05012026) ⭐ **NOVO**
16. [Code Interpreter para Cálculos](#-code-interpreter-para-cálculos-novo---06012026) ⭐ **NOVO**
17. [UI/UX - Menu Drawer e Comandos](#-uiux---menu-drawer-e-comandos-novo---07012026) ⭐ **NOVO**
18. [Consultas Bilhetadas](#consultas-bilhetadas)
19. [Relatórios e Dashboards](#relatórios-e-dashboards)
20. [Aprendizado e Contexto](#aprendizado-e-contexto)
21. [Configurações e Observabilidade](#configurações-e-observabilidade)
22. [Exemplos de Uso Prático](#exemplos-de-uso-prático)
23. [APIs Externas Utilizadas](#apis-externas-utilizadas)
24. [Arquitetura e Serviços](#arquitetura-e-serviços)
...
- ✅ **Sincronização Bancária**: Sincronização de extratos e conciliação (07/01/2026)
- ✅ **Normalização Inteligente**: Mapeamento automático cliente→categoria (08/01/2026)
- ✅ **Streaming de Respostas**: Respostas em tempo real (05/01/2026)
- ✅ **Busca Semântica**: Assistants API para legislação (05/01/2026)
- ✅ **Cálculos com Explicação**: Code Interpreter para cálculos complexos (06/01/2026)
- ✅ **UI/UX Moderna**: Menu drawer e comandos de voz/texto (07/01/2026)
...
8. **Bancário** - Sincronização, conciliação, despesas (07/01/2026)
9. **Normalização** - Mapeamento cliente→categoria (08/01/2026)
10. **Consultas Bilhetadas** - Aprovação, rejeição, execução
11. **Relatórios** - Dashboards, resumos, fechamentos
12. **Aprendizado** - Regras personalizadas, consultas salvas
13. **Legislação** - Busca semântica com Assistants API (05/01/2026)
14. **Cálculos** - Code Interpreter para cálculos complexos (06/01/2026)
...
- **Fonte principal**: SQLite (`chat_ia.db`, tabela `nesh_chunks`)
- **Fallback**: `nesh_chunks.json` (arquivo grande, usado só se o SQLite não estiver populado)
- **Opcional (28/01/2026)**: **HF/FAISS** para busca semântica por descrição em cima do SQLite

**Flags úteis no `.env`:**
- `NESH_HF_ENABLED=true|false`
- `NESH_HF_INDEX_DIR=/app/data/nesh_hf_index`
- `NESH_HF_EMBED_MODEL=intfloat/multilingual-e5-base`
- `NESH_LOG_SOURCE=true` (log no backend: `NESH fonte=HF|SQLITE|JSON`)
- `NESH_SHOW_SOURCE_IN_RESPONSE=true` (inclui `[NESH_META:{...}]` no rodapé da resposta)

**Gerar índice HF/FAISS (Docker):**
...
**Documentação completa:** `docs/integracoes/INTEGRACAO_BANCO_BRASIL.md`

---

## 💰 Sincronização e Conciliação Bancária (NOVO - 07/01/2026)

### Visão Geral

Sistema completo de sincronização de extratos bancários e conciliação de lançamentos, permitindo:
- Sincronizar extratos do Banco do Brasil e Santander para SQL Server
- Classificar lançamentos vinculando-os a tipos de despesa e processos
- Distribuir impostos de importação automaticamente
- Consultar movimentações bancárias diretamente do banco de dados

### Sincronizar Extratos Bancários

**Função:** `sincronizar_extrato_bancario` (via API `/api/banco/sincronizar`)

**Quando usar:** Quando o usuário pedir para sincronizar extratos bancários ou importar movimentações

**Exemplos de uso no chat:**
- "sincronizar extrato do banco"
- "importar movimentações do Santander"
- "sincronizar extratos dos últimos 30 dias"

**Parâmetros:**
- `banco`: "BB" ou "SANTANDER" (padrão: "BB")
- `agencia`: Agência (obrigatório para BB, opcional para Santander)
- `conta`: Conta (obrigatório para BB, opcional para Santander)
- `data_inicio`: Data inicial (formato: YYYY-MM-DD, opcional)
- `data_fim`: Data final (formato: YYYY-MM-DD, opcional)
- `dias_retroativos`: Número de dias para buscar (padrão: 7)

**Funcionalidades:**
- ✅ Detecção automática de duplicatas usando hash SHA-256
- ✅ Detecção automática de processos nas descrições de transações
- ✅ Suporte a múltiplas contas configuradas
...
- Novos lançamentos inseridos
- Duplicatas detectadas (puladas)
- Erros (se houver)
- Processos detectados automaticamente

### Conciliação Bancária

**Função:** `classificar_lancamento` (via API `/api/banco/classificar-lancamento`)

**Quando usar:** Quando o usuário pedir para classificar lançamentos bancários ou conciliar despesas

**Exemplos de uso no chat:**
- "quero conciliar banco"
- "classificar lançamento 123"
- "vincular despesa ao processo DMD.0083/25"

**Parâmetros:**
- `id_movimentacao`: ID do lançamento bancário
- `classificacoes`: Array de classificações (tipo de despesa + processo + valor)
- `distribuicao_impostos`: Distribuição de impostos de importação (opcional)
- `processo_referencia`: Processo para impostos (quando não há classificações)

**Funcionalidades:**
- ✅ Suporte a múltiplas classificações por lançamento (split)
- ✅ Distribuição automática de impostos de importação (II, IPI, PIS, COFINS)
- ✅ Validação de valores (soma não pode exceder valor total)
- ✅ Detecção automática de lançamentos de impostos

**Catálogo de Despesas:**
- 23 tipos de despesa pré-cadastrados (Frete Internacional, Seguro, AFRMM, Multas, Taxas Siscomex, etc.)
- Consultar via `/api/banco/tipos-despesa`

**Documentação completa:** `docs/funcionalidades/CATALOGO_DESPESAS_PADRAO.md`
...
**Formato incorreto (pode não funcionar):**
- ❌ "maike coloca como regra que o alh tambem pode ser chamado de alho"
- ❌ "maike alho é ALH"

**💡 Dica:** Use o formato "X vai ser Y" para garantir que a IA detecte corretamente como mapeamento cliente→categoria.

**Documentação completa:** `docs/funcionalidades/NORMALIZACAO_TERMOS_CLIENTE.md`

---

## 📡 Streaming de Respostas (NOVO - 05/01/2026)

### Visão Geral

Endpoint `/api/chat/stream` que envia respostas da IA em tempo real usando Server-Sent Events (SSE), melhorando significativamente a experiência do usuário.
...
**💡 Dica:** O frontend usa automaticamente streaming quando disponível, melhorando a experiência do usuário.

---

## 📚 Assistants API para Legislação (NOVO - 05/01/2026)

### Visão Geral

Integração com OpenAI Assistants API para busca semântica de legislação usando RAG (Retrieval-Augmented Generation), permitindo buscas inteligentes que entendem contexto e significado.

### Como Funciona

1. **Upload de Legislação**: Legislações são enviadas para a Assistants API
2. **Indexação Automática**: A API indexa automaticamente os documentos
3. **Busca Semântica**: Buscas entendem contexto e significado, não apenas palavras-chave
4. **Resposta Contextualizada**: Respostas incluem trechos relevantes da legislação

### Como Usar

**Exemplos de uso no chat:**
- "buscar legislação sobre importação de produtos agrícolas"
- "qual a legislação sobre NCM 90041000?"
- "buscar normas sobre DUIMP"

**Funcionalidades:**
- ✅ Busca semântica inteligente
- ✅ Respostas contextualizadas com trechos da legislação
- ✅ Suporte a múltiplos documentos
- ✅ Upload e indexação automática

**Documentação completa:** `docs/integracoes/ASSISTANTS_API_LEGISLACAO.md`

---

## 🧮 Code Interpreter para Cálculos (NOVO - 06/01/2026)

### Visão Geral

Integração com Code Interpreter da OpenAI (via Responses API) para cálculos de impostos e outros cálculos complexos com explicação detalhada passo a passo.

### Como Funciona

**Sistema Híbrido:**
- **Cálculos rápidos**: Usam Python local (`CalculoImpostosService`)
- **Cálculos com explicação**: Usam Code Interpreter da OpenAI
...
**Quando usar Code Interpreter:**
- Quando o usuário pedir explicação detalhada
- Quando o usuário pedir fórmulas
- Quando o usuário pedir passo a passo

**Quando usar cálculo local:**
- Quando o usuário pedir apenas o resultado
- Quando não pedir explicação

**Funcionalidades:**
- ✅ Cálculo completo de impostos (II, IPI, PIS, COFINS)
- ✅ Explicação detalhada passo a passo
- ✅ Fórmulas e bases de cálculo
- ✅ Formatação educativa

**Documentação completa:** `docs/funcionalidades/CODE_INTERPRETER_CALCULO_IMPOSTOS.md`

---

## 🎨 UI/UX - Menu Drawer e Comandos (NOVO - 07/01/2026)

### Visão Geral

Redesign completo da interface substituindo múltiplos botões no header por um menu drawer lateral elegante, com sistema de detecção de comandos de voz/texto.
...
**Vantagens:**
- ✅ Resposta instantânea (sem passar pela IA)
...
### Versão 1.9 (08/01/2026)
- ✅ Adicionada seção **Sincronização e Conciliação Bancária**
- ✅ Adicionada seção **Normalização de Termos Cliente → Categoria**
- ✅ Adicionada seção **Streaming de Respostas**
- ✅ Adicionada seção **Assistants API para Legislação**
- ✅ Adicionada seção **Code Interpreter para Cálculos**
- ✅ Adicionada seção **UI/UX - Menu Drawer e Comandos**
- ✅ Atualizado índice com novas funcionalidades
- ✅ Atualizada lista de características principais

### Versão 1.8 (06/01/2026)
- ✅ Adicionada seção **Banco do Brasil**
- ✅ Atualizada seção **Santander Open Banking**
