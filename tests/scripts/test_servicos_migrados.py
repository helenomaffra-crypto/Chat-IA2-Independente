...
- NCMService (buscar_ncms_por_descricao, sugerir_ncm_com_ia, detalhar_ncm, etc.)
...
def executar_testes_ncm_service():
    """Executa testes do NCMService"""
    print("\n" + "="*70)
    print("🔍 TESTANDO NCMSERVICE")
    print("="*70)
    
    try:
        from test_ncm_service import (
            test_buscar_ncms_por_descricao,
            test_sugerir_ncm_com_ia,
            test_detalhar_ncm,
            test_buscar_nota_explicativa_nesh,
            test_baixar_nomenclatura_ncm,
            test_integracao_chat_service as test_integracao_ncm
        )
        ...
        print("\n✅ Todos os testes do NCMService passaram!")
        return True
        
    except Exception as e:
        print(f"\n❌ Erro nos testes do NCMService: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def main():
    ...
    print("   (test_consulta_service.py, test_processo_list_service.py e test_ncm_service.py)")
    ...
    # Testar NCMService
    resultados.append(("NCMService", executar_testes_ncm_service()))
